...
export default function Coffee() {
  ...
  const [realtimeState, setRealtimeState] = React.useState<CoffeeRealtimeState>({
    step: 0,
  ...
  const pushRealtimeState = async (nextState: CoffeeRealtimeState) => {
    if (!roomId) return;
  ...
  const handleOrder = (type: CoffeeActionType) => {
    ...
    if (isRealtimeEnabled) {
      ...
      pushRealtimeState({
        ...
        resultBoom: realtimeState.resultBoom,
      });
      return;
    }
  ...
  const handleStepWithSync = (type: 'next' | 'prev') => {
    ...
    if (isRealtimeEnabled) {
      ...
      pushRealtimeState({
        ...
        resultBoom: type === 'prev' ? undefined : realtimeState.resultBoom,
      });
      return;
    }
  ...
  const handleCreateRoom = async () => {
    const state: CoffeeRealtimeState = {
      step: 0,
      orderState: initialCoffeeState,
    };
