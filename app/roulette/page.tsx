...
import { RouletteRealtimeState, RouletteSpinPlan } from '@/lib/realtime/types';
...
export default function Coffee() {
  ...
  const [realtimeState, setRealtimeState] = React.useState<RouletteRealtimeState>({
    step: 0,
  ...
  const pushRealtimeState = async (nextState: RouletteRealtimeState) => {
    if (!roomId) return;
  ...
  const handleStepWithSync = (type: 'next' | 'prev') => {
    ...
    if (isRealtimeEnabled) {
      ...
      const spinPlan: RouletteSpinPlan | undefined =
        nextStep === 2 && currentOrderState.total.length > 0
          ? {
              initialSpeed: Math.random() * 20 + 5,
              deceleration: Math.random() * 0.01 + 0.98,
              spinTime: 5000,
            }
          : undefined;
  ...
  const handleCreateRoom = async () => {
    const state: RouletteRealtimeState = {
      step: 0,
      orderState: initialRouletteState,
    };
  ...
  useEffect(() => {
    ...
    getRoom<RouletteRealtimeState>(roomId).then(room => {
      if (!mounted || !room) return;
      setRealtimeState(room.game_state);
    });

    const unsubscribe = subscribeRoomState<RouletteRealtimeState>({
      roomId,
      onState: state => {
        if (!mounted) return;
    ...
    <div className="relative">
      ...
      >
        <Container curStep={currentStep}>
          ...
          <Loading handleStep={handleStepWithSync} spinPlan={realtimeState.spinPlan} />
        </Container>
      </RouletteContext.Provider>
    </div>
  );
